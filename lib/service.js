  ...
  async convertChangesets(){
    ...
    for (let cs of changesets) {
      ...
      cs.Elements?.split(",")?.forEach(e => {
        let [type, name] = e.split(":")
        let element = Entity.find(`tag:element prop:type=${type}`)
        if(element){
          newCS.rel(element, "element")
        } else {
          //console.log(`${newCS.id}: Could not find element of type ${type} and name ${name}`)
        }
      })
    }
  }
