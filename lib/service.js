...
class SysService {
  ...
  async convertSprints() {
    ...
    (await this.get("iterations/all")).forEach(r => {
      ...
      if (!sprint) {
        sprint = new Entity().tag("sprint")
          .prop("id", id)
          .prop("startDate", r.StartDate.substr(0, 10))
          .prop("endDate", r.EndDate.substr(0, 10))
          .prop("title", r.Title)
      }
      sprint.storyPointsPlanned = r.PlannedStoryPoints
    });
  }
  ...
  async convertReleases() {
    ...
    console.log("Converting legacy hotfixes");
  ...
  async convertTags() {
    ...
    for (let t of tags) {
      ...
      if (!tag) {
        tag = new Entity().tag("tag")
                          .prop("id", t.tag)
      }
      tag.prop("hideInGrids", t.hideInGrids)
    ...
    (await this.get("assignment/search/:open|!:open/false/true/true")).forEach(a => {
      ...
      a.Relations.forEach(r => {
        ...
        switch(r.Type){
          ...
          case "Minifix":
            Entity.find(`tag:hotfix prop:"id=${r.Id}"`)?.rel(aNew, "issue")
            break;
  ...
  async convertRunbook() {
    ...
    for(let actionType of Entity.search("tag:actiontype")){
      switch(actionType.name){
        case "ApplyLabels": actionType.contextType = "instance"; break;
        case "BackupLayerFile": actionType.contextType = "instance"; break;
        case "BackupModel2012": actionType.contextType = "instance"; break;
        case "CompileModel": actionType.contextType = "instance"; break;
        case "CompileModels": actionType.contextType = "instance"; break;
        case "CopyLayerFromOtherInstance": actionType.contextType = "instance"; break;
        case "CopyModelFromOtherInstance": actionType.contextType = "instance"; break;
        case "CreateDeployablePackage": actionType.contextType = "instance"; break;
        case "DeleteAOI": actionType.contextType = "instance"; break;
        case "Deploy-Reports-AX7": actionType.contextType = "instance"; break;
        case "ExportModel": actionType.contextType = "instance"; break;
        case "GetURL": actionType.contextType = "instance"; break;
        case "ImportModel": actionType.contextType = "instance"; break;
        case "InitializeModelStore": actionType.contextType = "instance"; break;
        case "InstallFromSC": actionType.contextType = "instance"; break;
        case "RemoveLayerFile": actionType.contextType = "instance"; break;
        case "RunBCCommand": actionType.contextType = "instance"; break;
        case "SetMaintenanceMode": actionType.contextType = "instance"; break;
        case "StartAOS": actionType.contextType = "instance"; break;
        case "StopAOS": actionType.contextType = "instance"; break;
        case "SynchronizeModel": actionType.contextType = "instance"; break;
        case "SynchronizeModels": actionType.contextType = "instance"; break;
        case "UninstallModel": actionType.contextType = "instance"; break;
        case "UpdateFromSC": actionType.contextType = "instance"; break;
      }
    }
  }
  ...
  async convertMinifix() {
    ...
    for (let m of minifix) {
      let minifix = Entity.findOrCreate(`tag:hotfix prop:"id=${m.Num}"`)
        ...
        .prop("timestamp", m.PackedDate.slice(0, -5))
      ...
      Entity.find(`tag:release prop:id=${parseInt(m.Num)}`)?.rel(minifix, "hotfix")
    }
  }
  ...
  async convertDocs() {
    ...
    let docs = await this.get("docs/all")
    let ids = []
    let foundOne = false;
    for (let doc of docs) {

      let id = doc.Title.replace(/^\s+|\s+$/g, '') // trim
                        .toLowerCase()
    ...
    if(foundOne){
      let indexDoc = ids.map(id => `* [[${id}]]`).join("\n")
      Entity.findOrCreate(`tag:wiki prop:"id=legacydocs"`)
            .tag("wiki")
            .prop("id", "legacydocs")
            .prop("body", indexDoc)
            .removeProp("html")
  ...
  async convertWiki() {
    console.log("Converting wiki");

    let docs = await this.get("docs/wiki")
    let ids = []
    let foundOne = false;

    let genId = (id) => id.replace(/^\s+|\s+$/g, '') // trim
                          .toLowerCase()
                          .replace(/[^a-z0-9 -]/g, '') // remove invalid chars
                          .replace(/\s+/g, '-') // collapse whitespace and replace by -
                          .replace(/-+/g, '-'); // collapse dashes
    ...
    for (let doc of docs) {
      ...
      let id = doc.filename == "start.txt" ? "index" 
             : genId(doc.filename.slice(0, -4))
      ...
      new Entity()
        .tag("wiki")
        .prop("id", id)
        .prop("title", id.charAt(0).toUpperCase() + id.slice(1))
        .prop("body", doc.content.replace(/\=\=\=\=\=\=/g, "#")
                                 .replace(/\=\=\=\=\=/g, "##")
                                 .replace(/\=\=\=\=/g, "###")
                                 .replace(/\=\=\=/g, "####")
                                 .replace(/[\#]+\n/g, "\n")
                                 ...
                                 .replace(/\^\n/g, "|\n| -------- | -------- |\n")
                                 .replace(/\[\[\|\]\]/)
                                 .replace(/(\[\[(http[a-zA-Z0-9\:\/\.\-\%\_\#\=\?\+\&\"]+)\|([a-zA-Z0-9\sæøåÆØÅ\.]+)\]\])/g, (group, p1, p2, p3, p4, p5) => `[${p3}](${p2})`)
                                 .replace(/(\[\[([a-zA-Z0-9\-\_æøåÆØÅ]+)\|([a-zA-Z0-9\-æøåÆØÅ\.\s\(\)\,\'\>]+)\]\])/g, (group, p1, p2, p3, p4, p5) => `[${p3}](/wiki/${genId(p2)})`))
        .removeProp("html")
    ...
    if(foundOne){
      let indexDoc = ids.map(id => `* [[${id}]]`).join("\n")
      Entity.findOrCreate(`tag:wiki prop:"id=legacywiki"`)
            .tag("wiki")
            .prop("id", "legacywiki")
            .prop("body", indexDoc)
            .removeProp("html")
  ...
  async convertNotes(){
    ...
    let notes = await this.get("assignment/notes")

    let handledIssues = new Set()

    for(let note of notes){
      let issue = Entity.find(`tag:issue prop:legacyid=${note.AssignmentNum}`)
      if(!issue || handledIssues.has(issue.id)) continue;
      handledIssues.add(issue.id)
      let id = `issue-${issue.id}`
      let wiki = Entity.find(`tag:wiki prop:"id=${id}"`) || new Entity().tag("wiki").prop("id", id)
      
      wiki.title = `Issue ${issue.id}`
      wiki.body = notes.filter(n => n.AssignmentNum == note.AssignmentNum)
                       .map(n => {
                          let title = n.Note.split("\n")[0]
                          if(title.length > 80) title = title.substr(0, 74) + " [...]"
                          return `## ${title}\n\nNote written by [${n.Userid.toLowerCase()}](/setup/users/${n.Userid.toLowerCase()}) on ${n.Date.substr(0, 10)}\n\n${n.Note}`
                        })
                       .join("\n\n");
      wiki.removeProp("html")
    }
  }

  async convertForumComments(){
    console.log("Converting forum comments");

    let comments = await this.get("forum/comments")

    let handledThreads = new Set()

    for(let comment of comments){
      let thread = Entity.find(`tag:forumthread prop:id=${comment.ProjectRefId}`)
      if(!thread || handledThreads.has(thread.id)) continue;
      handledThreads.add(thread.id)
      let id = `forum-thread-${thread.id}`
      let wiki = Entity.find(`tag:wiki prop:"id=${id}"`) || new Entity().tag("wiki").prop("id", id)
      
      wiki.title = `Forum thread ${thread.id}`
      wiki.body = `Title: ${thread.title}<br>Author: ${thread.author}<br>[Open thread](/forum/thread/${thread.id})\n\n`
                + comments.filter(n => n.ProjectRefId == comment.ProjectRefId)
                       .map(n => {
                          return `## Comment by [${n.UserId.toLowerCase()}](/setup/users/${n.UserId.toLowerCase()}) on ${n.PostedDateTime.substr(0, 10)}\n\n${n.Comment}`
                        })
                       .join("\n\n");
      wiki.removeProp("html")
    }
